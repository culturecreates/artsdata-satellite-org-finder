class OrgVignette extends HTMLElement {
  set org(org) {
    ...
    this.innerHTML = `
      ...
      <a ${org.id[0] != "K" ? "" : "href='http://kg.artsdata.ca/resource/" + encodeURIComponent(org.id) + "'"} class="${org.match && "list-group-item-primary" } list-group-item list-group-item-action d-flex justify-content-between align-items-start">
      <div class="ms-2 me-auto">
      <div class="fw-bold">${encodeHTMLEntities(truncate(org.name))}</div>
      ${org.type[0].name} ${org.description && " - "} ${encodeHTMLEntities(org.description)}
      </div>
